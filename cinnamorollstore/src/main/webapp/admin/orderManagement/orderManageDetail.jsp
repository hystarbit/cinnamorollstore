		...
		<div class="mypage-main">
			...
			<div class="item-info">
				...
				<form class="order-user-info" method="post" id="statusForm"
					...
					action="${path}/admin/order/detail/edit.do">
					<input type="hidden" name="order_number"
						value="${order.order_number }"> <input type="hidden"
						name="delivery_number" value="${order.delivery_number }">
					<input type="hidden" id="selectedPaymentStatus"
						name="payment_status" value="${order.payment_status }"> <input
						type="hidden" id="selectedOrderStatus" name="order_status"
						value="${order.order_status }"> <input type="hidden"
						id="selectedDeliveryCompany" name="delivery_company"
				...
				<div class="order-item-list" style="border: 1px solid #333;">
					<table style="width: 100%;">
							...
							<td class="table-right border-free">
								...
								<ul class="dropdown-menu" style="min-width: 50px;">
									<li><a class="dropdown-item p" href="#"
										onclick="selectValue('selectedPaymentStatus','입금 전')">입금 전</a></li>
									<li><a class="dropdown-item p" href="#"
										onclick="selectValue('selectedPaymentStatus','결제 완료')">결제
											완료</a></li>
								</ul>
							</td>
							<td class="table-left black border-free">주문 처리 상태</td>
							<td class="table-right border-free">
								<button class="order-button dropdown-toggle" id="order"
									type="button" data-bs-toggle="dropdown" aria-expanded="false">${order.order_status }
								</button>
								<ul class="dropdown-menu" style="min-width: 50px;">
									<li><a class="dropdown-item o" href="#"
										onclick="selectValue('selectedOrderStatus','주문 확인 전')">주문
											확인 전</a></li>
									<li><a class="dropdown-item o" href="#"
										onclick="selectValue('selectedOrderStatus','주문 확인')">주문 확인</a></li>
									<li><a class="dropdown-item o" href="#"
										onclick="selectValue('selectedOrderStatus','배송 준비 중')">배송
											준비 중</a></li>
									<li><a class="dropdown-item o" href="#"
										onclick="selectValue('selectedOrderStatus','배송 중')">배송 중</a></li>
									<li><a class="dropdown-item o" href="#"
										onclick="selectValue('selectedOrderStatus','배송 완료')">배송 완료</a></li>
								</ul>
							</td>
							<td class="table-left black border-free"></td>
						...
						<c:if
							test="${order.order_status eq '배송 준비 중' || order.order_status eq '배송 중' 
							|| order.order_status eq '배송 완료'}">
							<tr>
								<td class="table-left black border-free">운송장 번호</td>
								<td class="table-right border-free"><c:choose>
										<c:when test="${empty order.delivery_company }">
											<button class="order-button dropdown-toggle" id="company"
												type="button" data-bs-toggle="dropdown"
												aria-expanded="false">택배 회사</button>
											<ul class="dropdown-menu" style="min-width: 50px;">
												<li><a class="dropdown-item c" href="#"
													onclick="selectValue('selectedDeliveryCompany','우체국')">우체국</a></li>
												<li><a class="dropdown-item c" href="#"
													onclick="selectValue('selectedDeliveryCompany','한진')">한진</a></li>
												<li><a class="dropdown-item c" href="#"
													onclick="selectValue('selectedDeliveryCompany','롯데')">롯데</a></li>
												<li><a class="dropdown-item c" href="#"
													onclick="selectValue('selectedDeliveryCompany','로젠')">로젠</a></li>
												<li><a class="dropdown-item c" href="#"
													onclick="selectValue('selectedDeliveryCompany','CJ 대한통운')">CJ
														대한통운</a></li>
											</ul>
										</c:when>
										<c:otherwise>
											<button class="order-button" id="company" type="button"
												data-bs-toggle="dropdown" aria-expanded="false">
												${order.delivery_company }</button>
										</c:otherwise>
									</c:choose></td>

								<td class="table-right border-free" colspan="3"><c:choose>
										<c:when test="${order.delivery_number != 0 }">
											<input type="text" value="${order.delivery_number }"
				...
				<script>
					...
					function toggleEditMode(){
						const addressField = document.getElementById('order_address');
									...
									<tr>
										...
										<c:choose>
											<c:when test="${not empty orderItem.application_number && orderItem.application_number != 0}">
												<td>
													<button class="order-check" style="width: auto;"
													onclick="location.href='${path }/admin/order/return/detail.do?application_number=${orderItem.application_number}'">
														현황
													</button>
												</td>
											</c:when>
											<c:otherwise>
												<td>
													<button class="order-check" style="width: auto;"
													onclick="location.href='${path }/admin/order/return/request.do?order_item_number=${orderItem.order_item_number }'">
														신청
													</button>
												</td>
											</c:otherwise>
